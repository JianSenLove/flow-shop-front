	...
	<view>
		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view class="cen">
					<view class="top">
						<text class="name" v-if="addressData">{{addressData.name}}</text>
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<view class="g-item" v-for="(itme,index) in List" :key="index">
				<image :src="itme.ioc"></image>
				<view class="right">
					<text class="title clamp">{{itme.title}}</text>
					<text class="spec">{{itme.specsname}} {{itme.fitname}}</text>
					<view class="price-box">
						<text class="price">￥{{itme.price}}</text>
						<text class="number">x {{itme.mun}}</text>
					</view>
				</view>
			</view>
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip">{{price-total>0 ? price-total:"免运费"}}</text>
			</view>
	...
	export default {
		...
		methods: {
			loadData(){
				...
				api.post('Cart/cartList', {"uid": this.userInfo.id,"id":this.cartid }).then(res => {
					// console.log(res)
					let list=res.data;
					this.List=list;
					let price = 0;
